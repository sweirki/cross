...
exports.calculateCompositionMetrics = calculateCompositionMetrics;
function round(value) {
    return Number(value.toFixed(4));
}
function calculateCompositionMetrics(rows, columns, occupied, clusters) {
    ...
    const mirroredHorizontal = new Set(occupied.map((cell) => `${cell.position.row}:${columns - 1 - cell.position.col}`));
    const mirroredVertical = new Set(occupied.map((cell) => `${rows - 1 - cell.position.row}:${cell.position.col}`));
