...
exports.validatePuzzle = validatePuzzle;
...
function validatePuzzle(puzzle) {
    ...
    for (const cell of puzzle.cells) {
        if (cells.has(cell.id))
            issues.push(issue("DUPLICATE_CELL_ID", `Duplicate cell ID: ${cell.id}.`, { cellId: cell.id }));
        cells.set(cell.id, cell);
        const key = `${cell.position.row}:${cell.position.col}`;
        if (positions.has(key))
            issues.push(issue("DUPLICATE_POSITION", `Duplicate cell position: ${key}.`, { cellId: cell.id }));
        positions.add(key);
    ...
    for (const equation of puzzle.equations) {
        ...
        const arithmetic = (0, ArithmeticEngine_1.applyArithmetic)(SYMBOL_TO_OPERATOR[equation.operator], left.solution, right.solution, OperatorRules_1.DEFAULT_ARITHMETIC_POLICY);
