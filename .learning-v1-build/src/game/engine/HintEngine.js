...
exports.getLogicalHint = getLogicalHint;
const solver_1 = require("../solver");
function getLogicalHint(puzzle, session) {
    if (session.puzzleId !== puzzle.id)
        throw new Error("Session and puzzle do not match.");
    const solved = (0, solver_1.solvePuzzle)(puzzle, { solutionLimit: 1, includeTrace: false });
    if (solved.firstSolution === null)
        return null;
    const solution = new Map(solved.firstSolution.map((item) => [item.cellId, item.value]));
    const editable = puzzle.cells
        .filter((cell) => cell.kind === "number" && cell.editable)
