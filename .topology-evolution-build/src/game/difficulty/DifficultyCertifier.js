...
exports.certifyDifficulty = certifyDifficulty;
const PuzzleSolver_1 = require("../solver/PuzzleSolver");
...
function maximumDecisionDepth(trace) {
    let depth = 0;
    let maximum = 0;
...
function metricVector(puzzle, trace) {
    const selections = trace.filter((event) => event.kind === "select");
    const hiddenCount = puzzle.cells.filter((cell) => cell.kind === "number" && !cell.given).length;
    ...
    const information = widths.length === 0
        ? 0
        : widths.reduce((sum, width) => sum + Math.max(0, Math.log2(startingChoices / Math.max(1, width))), 0) / widths.length;
    const characteristics = new Set();
    if (forced > 0)
        characteristics.add("forced");
    if (widths.some((width) => width > 1))
        characteristics.add("branch");
    if (puzzle.numberBank.length !== startingChoices)
        characteristics.add("multiplicity");
    const numberParticipation = new Map();
...
function difficultyScore(puzzle, metrics, visitedNodes) {
    const hiddenCount = puzzle.cells.filter((cell) => cell.kind === "number" && !cell.given).length;
    const selections = Math.max(1, metrics.deductionCount + (metrics.branchingFactor > 1 ? Math.max(1, metrics.proofDepth - metrics.deductionCount) : 0));
    const forcedRatio = Math.min(1, metrics.deductionCount / selections);
    const raw = hiddenCount * 1.5 +
...
function certifyDifficulty(puzzle) {
    const solved = (0, PuzzleSolver_1.solvePuzzle)(puzzle, { solutionLimit: 2, includeTrace: true });
    if (solved.status !== "unique" || !solved.searchExhausted) {
        throw new Error(`Difficulty certification requires a proven unique puzzle; solver status was ${solved.status}.`);
    }
    const metrics = metricVector(puzzle, solved.trace);
    const score = difficultyScore(puzzle, metrics, solved.visitedNodes);
    const hiddenCellCount = puzzle.cells.filter((cell) => cell.kind === "number" && !cell.given).length;
    const selections = solved.trace.filter((event) => event.kind === "select");
    const forcedMoveCount = selections.filter((event) => event.kind === "select" && event.candidates.length === 1).length;
