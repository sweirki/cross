...
const v1_1 = require("../../src/academy/v1");
...
function attempt(id, skillId, overrides = {}) {
    return {
...
const runtime = new v1_1.CrossMathAcademyRuntime();
let count = 0;
...
test("validates a prerequisite graph", () => (0, v1_1.validateSkillGraph)(graph));
test("rejects empty graphs", () => throws(() => (0, v1_1.validateSkillGraph)({ ...graph, skills: [] }), "Empty graph accepted."));
test("rejects duplicate skill IDs", () => throws(() => (0, v1_1.validateSkillGraph)({
    ...graph, skills: [...graph.skills, graph.skills[0]],
}), "Duplicate skill accepted."));
test("rejects missing prerequisites", () => throws(() => (0, v1_1.validateSkillGraph)({
    ...graph, skills: [{ id: "x", title: "X", prerequisiteIds: ["missing"] }],
}), "Missing prerequisite accepted."));
test("rejects self prerequisites", () => throws(() => (0, v1_1.validateSkillGraph)({
    ...graph, skills: [{ id: "x", title: "X", prerequisiteIds: ["x"] }],
}), "Self prerequisite accepted."));
test("rejects graph cycles", () => throws(() => (0, v1_1.validateSkillGraph)({
    schemaVersion: 1, id: "cycle", skills: [
        { id: "a", title: "A", prerequisiteIds: ["b"] },
        { id: "b", title: "B", prerequisiteIds: ["a"] },
    ],
}), "Cycle accepted."));
test("rejects invalid mastery thresholds", () => throws(() => (0, v1_1.validateSkillGraph)({
...
test("rejects unknown attempt skills", () => throws(() => (0, v1_1.validateAttempt)(graph, attempt("1", "missing")), "Unknown skill accepted."));
test("rejects invalid stars", () => throws(() => (0, v1_1.validateAttempt)(graph, attempt("1", "addition", { stars: 4 })), "Invalid stars accepted."));
...
test("scores perfect attempts highly", () => {
    const mastery = (0, v1_1.calculateMastery)(graph, [
...
test("penalizes hints and mistakes", () => {
    const clean = (0, v1_1.calculateMastery)(graph, [attempt("1", "addition")])[0].score;
    const assisted = (0, v1_1.calculateMastery)(graph, [attempt("1", "addition", { hintsUsed: 2, mistakes: 2 })])[0].score;
    assert(assisted < clean, "Hints and mistakes did not reduce mastery.");
});
test("weights recent attempts more heavily", () => {
    const improving = (0, v1_1.calculateMastery)(graph, [
        attempt("1", "addition", { completed: false, stars: 0 }),
        attempt("2", "addition"),
    ])[0].score;
    const declining = (0, v1_1.calculateMastery)(graph, [
        attempt("1", "addition"),
...
test("emits mastery band changes", () => {
    ...
    let latest = runtime.recordAttempt(graph, state, attempt("1", "addition"));
    state = latest.state;
    latest = runtime.recordAttempt(graph, state, attempt("2", "addition"));
    state = latest.state;
...
test("recommends the first available lesson", () => {
    ...
    if (recommendation.type === "lesson")
        equal(recommendation.skillId, "addition", "Wrong first skill.");
});
test("gates lessons behind prerequisites", () => {
    ...
    if (recommendation.type === "practice")
        equal(recommendation.skillId, "addition", "Wrong gated skill.");
});
test("unlocks dependent lessons after mastery", () => {
    ...
    if (recommendation.type === "lesson")
        assert(recommendation.skillId === "intersections" || recommendation.skillId === "subtraction", "Unexpected dependent skill.");
});
...
test("returns complete when all skills are mastered", () => {
    ...
    for (const skill of graph.skills)
        for (let i = 0; i < 3; i += 1)
            attempts.push(attempt(String(n++), skill.id));
...
test("replay is deterministic across input order", () => {
    ...
    equal(runtime.serialize(runtime.replay(graph, "p", rows)), runtime.serialize(runtime.replay(graph, "p", [...rows].reverse())), "Replay depends on input order.");
});
