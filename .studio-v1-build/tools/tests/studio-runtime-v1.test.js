...
const v1_1 = require("../../src/studio/v1");
let passed = 0;
...
function puzzle(id = "puzzle-001", status = "draft") {
    return {
        schemaVersion: 1,
        kind: "puzzle",
        id,
...
function pack(resources = [puzzle()]) {
    return {
        schemaVersion: 1,
        id: "studio-pack",
        version: "1.0.0",
...
const studio = new v1_1.CrossMathStudio();
const create = (resources = [puzzle()]) => studio.createProject("project-one", pack(resources), "1.0.0", 100).project;
...
test("rejects generated duplicate resources", () => {
    const duplicate = { generate: () => ({ resource: puzzle() }) };
...
test("captures generation failures in simulation", () => {
    const failing = { generate: (_seed, index) => { if (index === 1)
            throw new Error("boom"); return { resource: puzzle(`ok-${index}`) }; } };
