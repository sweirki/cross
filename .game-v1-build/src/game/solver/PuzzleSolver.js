...
exports.verifyUniqueSolution = verifyUniqueSolution;
...
function requireCell(cells, id, equationId) {
...
function buildModel(puzzle) {
    ...
    for (const tile of puzzle.numberBank) {
        if (tileIds.has(tile.id))
            throw new Error(`Duplicate number-bank tile ID: ${tile.id}.`);
        if (!Number.isInteger(tile.value))
            throw new Error(`Number-bank tile ${tile.id} must be an integer.`);
        tileIds.add(tile.id);
...
function equationSatisfied(equation, assignments) {
    ...
    if (left === undefined || right === undefined || result === undefined)
        return true;
    const evaluated = (0, ArithmeticEngine_1.applyArithmetic)(equation.operation, left, right, OperatorRules_1.DEFAULT_ARITHMETIC_POLICY);
    return evaluated.ok && evaluated.result === result;
}
function decrement(counts, value) {
...
function hasEquationSupport(equation, assignments, counts) {
    const ids = [equation.leftId, equation.rightId, equation.resultId];
    const missing = ids.filter((id) => !assignments.has(id));
    if (missing.length === 0)
        return equationSatisfied(equation, assignments);
    const values = [...counts.keys()].sort((a, b) => a - b);
    const working = new Map(assignments);
    function visit(index, remaining) {
        if (index === missing.length)
            return equationSatisfied(equation, working);
...
function candidateValues(cellId, assignments, counts, equations) {
    const candidates = [];
    for (const value of [...counts.keys()].sort((a, b) => a - b)) {
        ...
        if (equations.every((equation) => hasEquationSupport(equation, nextAssignments, nextCounts))) {
            candidates.push(value);
        }
    }
    return candidates;
}
function canonicalSolution(cells, assignments) {
    return cells
        .map((cell) => {
        const value = assignments.get(cell.id);
        if (value === undefined)
            throw new Error(`Missing solved value for ${cell.id}.`);
        return {
            cellId: cell.id,
...
function solvePuzzle(puzzle, options = {}) {
    ...
    for (const cell of model.numberCells) {
        if (cell.given)
            initialAssignments.set(cell.id, cell.value);
    }
    function emit(event) {
        if (!includeTrace)
            return;
        step += 1;
        trace.push({ step, ...event });
    }
    function search(assignments, counts) {
        if (solutionCount >= solutionLimit) {
            stoppedAtLimit = true;
            return;
        }
        visitedNodes += 1;
        if (!model.equations.every((equation) => hasEquationSupport(equation, assignments, counts))) {
            return;
        }
        ...
        const ranked = unresolved
            .map((cell) => ({
            cell,
            candidates: candidateValues(cell.id, assignments, counts, model.equationByCell.get(cell.id) ?? []),
        }))
        ...
        for (const value of selected.candidates) {
            ...
            if ((model.equationByCell.get(selected.cell.id) ?? []).every((equation) => hasEquationSupport(equation, nextAssignments, nextCounts))) {
...
function verifyUniqueSolution(puzzle, options = {}) {
