...
test("rejects unknown selections", () => {
  let threw = false;
  try { runtime.dispatch(puzzle, runtime.create(puzzle).state, { type: "select-tile", tileId: "missing" }); }
  catch { threw = true; }
...
test("rejects invalid time", () => {
  let threw = false;
  try { runtime.dispatch(puzzle, runtime.create(puzzle).state, { type: "advance-time", milliseconds: -1 }); }
  catch { threw = true; }
...
test("rejects corrupt persistence", () => {
  let threw = false;
  try { runtime.restore(puzzle, "{bad"); } catch { threw = true; }
  ...
  threw = false;
  try { runtime.restore(puzzle, JSON.stringify(valid)); } catch { threw = true; }
