import {
  ...
  TelemetryBuffer,
  createHint,
  summarizeDifficultyTelemetry,
  validateHintAgainstTrace,
  validateTelemetryPrivacy,
} from "../../src/generation";
import type {
  DeductionTrace,
  DifficultyTelemetrySession,
} from "../../src/generation";
...
const trace: DeductionTrace = Object.freeze({
  solved: true,
...
for (const level of [1, 2, 3, 4] as const) {
  ...
  check(validateHintAgainstTrace({
    puzzleId: "p-1",
    trace,
    solvedCellIds: ["a", "b", "x"],
    requestedLevel: level,
...
const deterministicA = createHint({ puzzleId: "p-1", trace, solvedCellIds: ["a", "b", "x"], requestedLevel: 2 });
const deterministicB = createHint({ puzzleId: "p-1", trace, solvedCellIds: ["x", "b", "a"], requestedLevel: 2 });
...
check(createHint({ puzzleId: "p-1", trace, solvedCellIds: ["a", "b", "c", "d", "e", "x"] }).kind === "puzzle-complete", "complete hint");
check(createHint({ puzzleId: "p-1", trace, solvedCellIds: [] }).kind === "narrow-candidates" || createHint({ puzzleId: "p-1", trace, solvedCellIds: [] }).kind === "focus-equation", "fallback step is supported");
const tampered = { ...createHint({ puzzleId: "p-1", trace, solvedCellIds: ["a", "b"], requestedLevel: 4 }), revealedValue: 999 };
check(!validateHintAgainstTrace({ puzzleId: "p-1", trace, solvedCellIds: ["a", "b"], requestedLevel: 4 }, tampered).valid, "tamper rejected");

const hintSession = new HintSession("p-1", trace);
...
const session: DifficultyTelemetrySession = {
  schemaVersion: 1,
  sessionId: "session-1",
  puzzleId: "p-1",
  difficulty: "hard",
...
const summary = summarizeDifficultyTelemetry(session);
...
check(validateTelemetryPrivacy(session).valid, "session privacy");
const unsafe = validateTelemetryPrivacy({ type: "unsafe", answer: 7, nested: { email: "x@y.test" } });
...
try { buffer.record({ type: "undo", atMs: -1 }); } catch { timestampRejected = true; }
