import {
  CrossMathCampaignExperience,
  type CampaignExperienceView,
} from "../../src/application/v1";
...
function truthy(value: unknown): void {
  if (!value) throw new Error(`Expected truthy value, received ${JSON.stringify(value)}.`);
}

function completeLesson(
  app: CrossMathApplicationRuntime,
  state: ReturnType<CrossMathApplicationRuntime["create"]>,
  lessonIndex: number,
  starsMode: "perfect" | "hinted" = "perfect",
) {
  const lesson = LEARNING_CONTENT.lessons[lessonIndex];
  if (lesson === undefined) throw new Error("Missing fixture lesson.");
  let next = state;
  for (const puzzleId of lesson.puzzleIds) {
    next = app.recordPuzzleCompleted(
      next,
      puzzleId,
      starsMode === "perfect" ? 2 : 7,
      starsMode === "perfect" ? 0 : 2,
      `2026-01-0${lessonIndex + 1}T00:00:00.000Z`,
    );
  }
  return next;
}

const app = new CrossMathApplicationRuntime();
const runtime = new CrossMathCampaignExperience();

function fresh(): CampaignExperienceView {
  return runtime.build(LEARNING_CONTENT, app.create("p1"));
}
...
test("includes every chapter", () => {
  equal(fresh().chapters.length, LEARNING_CONTENT.campaign.chapters.length);
});

test("includes every lesson exactly once", () => {
  const ids = fresh().chapters.flatMap((chapter) =>
    chapter.lessons.map((lesson) => lesson.id),
  );
  equal(ids, LEARNING_CONTENT.campaign.chapters.flatMap((chapter) => chapter.lessonIds));
});
...
test("calculates maximum stars", () => {
  const expected = LEARNING_CONTENT.lessons.reduce(
    (sum, lesson) => sum + lesson.puzzleIds.length * 3,
    0,
  );
...
test("recommends the first lesson initially", () => {
  equal(fresh().nextLessonId, LEARNING_CONTENT.lessons[0]?.id);
});
...
test("marks a completed chapter", () => {
  let state = completeLesson(app, app.create("p1"), 0);
  state = completeLesson(app, state, 1);
  equal(runtime.build(LEARNING_CONTENT, state).chapters[0]?.status, "completed");
});

test("activates the next chapter", () => {
  let state = completeLesson(app, app.create("p1"), 0);
  state = completeLesson(app, state, 1);
  equal(runtime.build(LEARNING_CONTENT, state).chapters[1]?.status, "active");
});
...
test("marks completed path segments", () => {
  const state = completeLesson(app, app.create("p1"), 0);
  equal(runtime.build(LEARNING_CONTENT, state).chapters[0]?.path[0]?.status, "completed");
});

test("calculates overall completion percentage", () => {
  let state = completeLesson(app, app.create("p1"), 0);
  state = completeLesson(app, state, 1);
  equal(runtime.build(LEARNING_CONTENT, state).completionPercent, 50);
});

test("finds lessons by ID", () => {
  equal(
    runtime.findLesson(LEARNING_CONTENT, "lesson-001-place-a-number")?.title,
    "Place Your First Number",
  );
});

test("returns null for an unknown lesson", () => {
  equal(runtime.findLesson(LEARNING_CONTENT, "missing"), null);
});
...
test("reports chapter unlock state", () => {
  ...
  equal(
    [
      runtime.isChapterUnlocked(
        LEARNING_CONTENT.campaign,
        LEARNING_CONTENT,
        state,
...
test("marks the campaign complete", () => {
  ...
  for (let index = 0; index < LEARNING_CONTENT.lessons.length; index += 1) {
    state = completeLesson(app, state, index);
  }
  const view = runtime.build(LEARNING_CONTENT, state);
  equal(
    [view.campaignCompleted, view.completionPercent, view.nextLessonId],
    [true, 100, null],
  );
});
