...
const assert = {
  ...
  throws(run: () => void): void {
    let threw = false;
    try { run(); } catch { threw = true; }
    if (!threw) throw new Error("Expected function to throw.");
  },
};
import { ACHIEVEMENTS, CrossMathProgressionRuntime } from "../../src/progression/v1";

const runtime = new CrossMathProgressionRuntime();
let passed = 0;
...
test("unknown achievement throws", () => assert.throws(() => runtime.achievement("missing" as never)));
