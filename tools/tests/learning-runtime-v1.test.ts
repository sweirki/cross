import { CrossMathGameRuntime } from "../../src/game/runtime";
import { CrossMathLearningRuntime, validateCampaignDefinition, validateLessonDefinition } from "../../src/learning/v1";
import type { CampaignDefinition, LessonDefinition, LessonState } from "../../src/types/LearningRuntime";
...
const lesson2: LessonDefinition = {
  schemaVersion: 1,
  id: "lesson-2",
...
const gameRuntime = new CrossMathGameRuntime();
const runtime = new CrossMathLearningRuntime();
let count = 0;
function test(name: string, body: () => void): void {
  body(); count += 1; console.log(`PASS ${name}`);
}

function completeLesson(definition: LessonDefinition = lesson): LessonState {
...
test("validates lesson definitions", () => {
  ...
  throws(() => validateLessonDefinition({ ...lesson, objectives: [] }), "Empty lesson accepted.");
  throws(() => validateLessonDefinition({
    ...lesson,
...
test("validates campaign definitions", () => {
  ...
  throws(() => validateCampaignDefinition({
    ...campaign,
...
test("rejects progress for locked lessons", () => {
  ...
  throws(
    () => runtime.recordLesson(campaign, runtime.createCampaign(campaign).state, lessonState),
    "Locked lesson progress accepted.",
  );
});
...
test("completes campaign and emits event", () => {
  ...
  const secondComplete: LessonState = {
