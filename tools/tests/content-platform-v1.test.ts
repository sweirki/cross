...
import { CrossMathContentPlatform } from "../../src/content/v1";
import type { ContentMigration, ContentPack, ContentResource } from "../../src/types/ContentPlatformRuntime";
...
const lesson = (status: ContentResource["status"] = "draft"): ContentResource => ({
  schemaVersion: 1,
  kind: "lesson",
  id: "addition-basics",
  version: "1.0.0",
...
const puzzle = (status: ContentResource["status"] = "draft"): ContentResource => ({
  schemaVersion: 1,
  kind: "puzzle",
  id: "puzzle-001",
  version: "1.0.0",
...
const pack = (resources: readonly ContentResource[] = [puzzle(), lesson()]): ContentPack => ({
  schemaVersion: 1,
  id: "starter-pack",
  version: "1.0.0",
  title: "Starter Pack",
...
const platform = new CrossMathContentPlatform();
...
test("warns for missing optional dependencies", () => {
  const value = { ...lesson(), dependencies: [{ kind: "puzzle" as const, id: "missing", optional: true }] };
...
test("rejects self dependencies", () => {
  const value = { ...puzzle(), dependencies: [{ kind: "puzzle" as const, id: "puzzle-001" }] };
...
const migration: ContentMigration = {
  kind: "lesson",
  fromSchemaVersion: 1,
  toSchemaVersion: 2,
  migrate(payload) {
    return { ...(payload as Record<string, unknown>), migrated: true };
  },
};
const migrating = new CrossMathContentPlatform([migration]);
test("migrates resources", () => {
  const result = migrating.migrateResource("starter-pack", lesson(), 2);
  equal(result.resource.schemaVersion, 2);
  equal((result.resource.payload as { migrated: boolean }).migrated, true);
...
test("rejects skipped migration definitions", () => throws(() => new CrossMathContentPlatform([{ ...migration, toSchemaVersion: 3 }]), "exactly one"));
test("rejects duplicate migration definitions", () => throws(() => new CrossMathContentPlatform([migration, migration]), "Duplicate migration"));
