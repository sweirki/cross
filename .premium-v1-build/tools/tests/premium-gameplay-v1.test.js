...
const puzzle2 = { ...puzzle, id: "premium-v2", difficulty: "medium" };
const library = { schemaVersion: 1, id: "lib", puzzles: [puzzle, puzzle2] };
const content = {
    templates: [{
...
const runtime = new v1_1.CrossMathPremiumRuntime();
let count = 0;
function test(name, body) { body(); count += 1; console.log(`PASS ${name}`); }
function solvedSession(hints = false) {
    ...
    if (hints)
        s = runtime.requestHint(puzzle, s, 1).session;
...
test("advances hints progressively", () => {
    let s = runtime.startSession(runtime.createProfile("p"), puzzle, 0).session;
    const h1 = runtime.requestHint(puzzle, s);
    s = h1.session;
    const h2 = runtime.requestHint(puzzle, s);
    equal(h1.hint?.level, 1, "Wrong first level.");
    equal(h2.hint?.level, 2, "Wrong second level.");
});
...
test("rejects bad hint levels", () => throws(() => runtime.requestHint(puzzle, runtime.startSession(runtime.createProfile("p"), puzzle, 0).session, 6), "Bad level accepted."));
...
test("records completed attempts", () => {
    const { p, s } = solvedSession();
    const r = runtime.completeAttempt(p, s, { concept: "addition", completedAt: "2026-07-28T10:00:00Z" });
    equal(r.profile.attempts.length, 1, "Attempt missing.");
    equal(r.profile.attempts[0]?.stars, 3, "Stars wrong.");
});
test("penalizes hinted solves", () => {
    const { p, s } = solvedSession(true);
    equal(runtime.completeAttempt(p, s, { completedAt: "2026-07-28T10:00:00Z" }).profile.attempts[0]?.stars, 2, "Hint penalty wrong.");
});
test("rejects incomplete attempts", () => {
    const p = runtime.createProfile("p");
    const s = runtime.startSession(p, puzzle, 0).session;
...
test("rejects cross-player attempts", () => {
    const { s } = solvedSession();
    throws(() => runtime.completeAttempt(runtime.createProfile("other"), s, { completedAt: "2026-07-28T10:00:00Z" }), "Cross-player accepted.");
});
test("rejects duplicate attempts", () => {
    const { p, s } = solvedSession();
    const a = runtime.completeAttempt(p, s, { completedAt: "2026-07-28T10:00:00Z" }).profile;
...
test("marks daily completion once", () => {
    const p = runtime.createProfile("p");
    const a = runtime.markDailyComplete(p, "2026-07-28");
    const b = runtime.markDailyComplete(a.profile, "2026-07-28");
    equal(a.profile.completedDailyDates.length, 1, "Date missing.");
    equal(b.events.length, 0, "Duplicate event emitted.");
});
test("summarizes statistics", () => {
    const { p, s } = solvedSession();
    const p2 = runtime.completeAttempt(p, s, { concept: "addition", completedAt: "2026-07-28T10:00:00Z" }).profile;
...
test("updates accessibility", () => {
    ...
    equal(r.profile.accessibility.reducedMotion, true, "Preference missing.");
    equal(r.events[0]?.type, "accessibility-updated", "Event missing.");
});
...
test("serializes profiles canonically", () => {
    const p = runtime.createProfile("p");
    equal(runtime.serializeProfile(runtime.restoreProfile(runtime.serializeProfile(p))), runtime.serializeProfile(p), "Profile roundtrip failed.");
});
...
test("replays deterministically", () => {
    const p = runtime.createProfile("p");
    const actions = [{ type: "advance-time", milliseconds: 1000 }, { type: "place", cellId: "a", tileId: "two" }];
    const a = runtime.replay(puzzle, p, actions, 0);
    const b = runtime.replay(puzzle, p, actions, 0);
