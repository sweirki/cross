...
const runtime = new CrossMathApplicationRuntime_1.CrossMathApplicationRuntime();
...
test("recommends the first lesson initially", () => equal(runtime.nextLesson(learningContent_1.LEARNING_CONTENT, runtime.create("p1"))?.id, learningContent_1.LEARNING_CONTENT.lessons[0]?.id));
test("unlocks lessons sequentially", () => {
    let state = runtime.create("p1");
    equal(runtime.isLessonUnlocked(learningContent_1.LEARNING_CONTENT, state, learningContent_1.LEARNING_CONTENT.lessons[1].id), false);
    state = runtime.recordPuzzleCompleted(state, learningContent_1.LEARNING_CONTENT.lessons[0].puzzleIds[0], 2, 0, "2026-01-01T00:00:00.000Z");
    equal(runtime.isLessonUnlocked(learningContent_1.LEARNING_CONTENT, state, learningContent_1.LEARNING_CONTENT.lessons[1].id), true);
});
test("selects daily puzzles deterministically", () => equal(runtime.dailyPuzzle(bundledLibrary_1.BUNDLED_LIBRARY.puzzles, "2026-01-01").id, runtime.dailyPuzzle(bundledLibrary_1.BUNDLED_LIBRARY.puzzles, "2026-01-01").id));
test("selects unfinished practice first", () => equal(runtime.practicePuzzle(bundledLibrary_1.BUNDLED_LIBRARY.puzzles, runtime.create("p1")).id, bundledLibrary_1.BUNDLED_LIBRARY.puzzles[0].id));
