...
exports.renderTopologyAscii = renderTopologyAscii;
...
function tokenFor(node, options) {
    if (node.kind === "number") {
...
function renderTopologyAscii(topology, options = {}) {
    ...
    const tokens = new Map(topology.nodes.map((node) => [
        `${node.position.row}:${node.position.column}`,
        tokenFor(node, options),
    ]));
    const width = Math.max(1, ...[...tokens.values()].map((token) => [...token].length));
    const lines = [];
