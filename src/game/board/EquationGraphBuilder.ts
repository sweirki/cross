...
import type {
  ...
  EquationGraphIntersection,
  EquationGraphValidationIssue,
  EquationGraphValidationResult,
  EquationVariable,
  VariableId,
} from "../../types/EquationGraph";

function variableId(nodeId: NodeId): VariableId {
  return `var-${nodeId}`;
}

function requireNode(
  nodes: ReadonlyMap<NodeId, TopologyNode>,
  ...
  if (node === undefined) {
    throw new Error(
      `Equation ${equation.id} references missing node ${nodeId}.`,
    );
  }
  return node;
}
...
function requireNumber(
  ...
  if (node.kind !== "number") {
    throw new Error(
      `Equation ${equation.id} expected a number at path index ${index}.`,
    );
  }
  return node;
}
...
function requireOperator(
  ...
  if (node.kind !== "operator") {
    throw new Error(
      `Equation ${equation.id} expected an operator at path index 1.`,
    );
  }
  return node;
}
 ...
 */
export function buildEquationGraph(topology: BoardTopology): EquationGraph {
  const nodesById = new Map(topology.nodes.map((node) => [node.id, node]));
  const participation = new Map<NodeId, string[]>();

  const equations: EquationGraphEquation[] = [...topology.equations]
  ...
  const variables: EquationVariable[] = topology.nodes
    .filter((node): node is NumberTopologyNode => node.kind === "number")
    .map((node) => ({
  ...
  const intersections: EquationGraphIntersection[] = variables
...
export function serializeEquationGraph(graph: EquationGraph): string {
  return JSON.stringify(graph);
}

function graphIssue(
  code: EquationGraphValidationIssue["code"],
  message: string,
  context: Pick<
    EquationGraphValidationIssue,
    "variableId" | "equationId"
  > = {},
): EquationGraphValidationIssue {
...
export function validateEquationGraph(
  ...
  const issues: EquationGraphValidationIssue[] = [];
  const variables = new Map<VariableId, EquationVariable>();
    ...
    if (variable.equationIds.length < 1 || variable.equationIds.length > 2) {
      issues.push(
        graphIssue(
          ...
          `Variable ${variable.id} must participate in one or two equations.`,
          { variableId: variable.id },
        ),
      );
    }
    ...
    if (equationIds.has(equation.id)) {
      issues.push(
        graphIssue(
          ...
          `Duplicate equation ID: ${equation.id}.`,
      ...
      if (!variables.has(id)) {
        issues.push(
          graphIssue(
            ...
            `Equation ${equation.id} references missing variable ${id}.`,
            { equationId: equation.id, variableId: id },
          ),
        );
      }
    }
  ...
  for (const id of equationIds) adjacency.set(id, new Set());
    ...
    if (
      ...
      issues.push(
        graphIssue(
          ...
          `Intersection ${intersection.variableId} is inconsistent.`,
          { variableId: intersection.variableId },
        ),
      );
      continue;
    }
  ...
  if (graph.equations.length > 1) {
    ...
    if (visited.size !== graph.equations.length) {
      issues.push(
        graphIssue(
          "DISCONNECTED_GRAPH",
          "Equation graph must form one connected component.",
        ),
      );
    }
  }
