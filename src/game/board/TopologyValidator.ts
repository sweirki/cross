...
import type {
  ...
  TopologyValidator,
} from "../validation/TopologyValidation";
...
function positionKey(node: TopologyNode): string {
  return `${node.position.row}:${node.position.column}`;
}

function issue(
  code: TopologyValidationIssue["code"],
  message: string,
  context: Pick<TopologyValidationIssue, "nodeId" | "equationId"> = {},
): TopologyValidationIssue {
...
function validatePathGeometry(
  ...
  return nodes.every((node, index) => {
    if (equation.orientation === "horizontal") {
      return (
        node.position.row === first.position.row &&
        node.position.column === first.position.column + index
      );
    }

    return (
      node.position.column === first.position.column &&
      node.position.row === first.position.row + index
    );
  });
}

export class DeterministicTopologyValidator
  ...
  validate(topology: BoardTopology): TopologyValidationResult {
    ...
    for (const node of topology.nodes) {
      if (nodesById.has(node.id)) {
        issues.push(
          issue("DUPLICATE_NODE_ID", `Duplicate node ID: ${node.id}.`, {
            nodeId: node.id,
          }),
        );
      } else {
      ...
      if (
        ...
        issues.push(
          issue(
            ...
            `Node ${node.id} lies outside the board.`,
            { nodeId: node.id },
          ),
        );
      }
    ...
    for (const equation of topology.equations) {
      ...
      if (signatures.has(signature)) {
        issues.push(
          issue(
            ...
            `Equation ${equation.id} duplicates an existing path.`,
            { equationId: equation.id },
          ),
        );
      }
      ...
      if (!missingReference && referenced.length === 5) {
        const validPattern = referenced.every(
          (node, index) => node.kind === EXPECTED_PATTERN[index],
        );
        ...
        if (!validPattern) {
          issues.push(
            issue(
              ...
              `Equation ${equation.id} does not follow number-operator-number-equals-number.`,
              { equationId: equation.id },
            ),
          );
        }
        ...
        if (!validatePathGeometry(equation, referenced)) {
          issues.push(
            issue(
              "INVALID_EQUATION_PATH",
              `Equation ${equation.id} is not a contiguous straight path.`,
              { equationId: equation.id },
            ),
          );
        }
      ...
      if (node.kind === "number") {
        if (counts.horizontal > 1 || counts.vertical > 1 || total > 2) {
          issues.push(
            issue(
              ...
              `Number node ${node.id} exceeds participation limits.`,
              { nodeId: node.id },
            ),
          );
        }
        ...
        if (total > 1) {
          issues.push(
            issue(
              ...
              `${node.kind} node ${node.id} is shared by equations.`,
              { nodeId: node.id },
            ),
          );
        }
        ...
        if (total !== 1) {
          issues.push(
            issue(
              "NODE_PARTICIPATION_EXCEEDED",
              `${node.kind} node ${node.id} must belong to exactly one equation.`,
              { nodeId: node.id },
            ),
          );
        }
    ...
    if (topology.equations.length > 1) {
      ...
      const equationsByNode = new Map<NodeId, string[]>();
      for (const equation of topology.equations) {
        for (const nodeId of equation.nodeIds) {
      ...
      if (visited.size !== topology.equations.length) {
        issues.push(
          issue(
            ...
            "All equations must form one connected component.",
          ),
        );
      }
    }
    ...
    if (genuineIntersections === 0) {
      issues.push(
        issue(
          ...
          "At least one horizontal/vertical number intersection is required.",
        ),
      );
    }
...
export const topologyValidator = new DeterministicTopologyValidator();

export function validateBoardTopology(
