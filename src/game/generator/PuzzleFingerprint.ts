...
import type { PuzzleFingerprints } from "../../types/IndustrialGeneration";
...
function byPosition(left: Cell, right: Cell): number {
  return left.position.row - right.position.row ||
    left.position.col - right.position.col ||
    left.id.localeCompare(right.id);
}

export function fingerprintPuzzle(puzzle: Puzzle): PuzzleFingerprints {
  ...
  const solutionPayload = JSON.stringify(
    cells.filter((cell) => cell.kind === "number").map((cell) => cell.solution),
  );
  return {
