...
import type { EquationFeedback, GameAction, GameHistoryAction, GameSession } from "../../types/Game";
import type {
  CrossMathGameRuntimeContract,
...
function assertState(puzzle: Puzzle, state: RuntimeState): void {
...
function completedEquationIds(before: readonly EquationFeedback[], after: readonly EquationFeedback[]): readonly string[] {
...
function incorrectEquationIds(feedback: readonly EquationFeedback[]): readonly string[] {
  return feedback.filter((item) => item.state === "incorrect").map((item) => item.equationId).sort();
}

function isHistoryAction(action: RuntimeAction): action is GameHistoryAction {
  return action.type === "place" || action.type === "remove" || action.type === "hint" ||
    action.type === "reset" || action.type === "undo" || action.type === "redo" ||
    action.type === "restore";
}

function isBoardMutation(action: RuntimeAction): action is GameAction {
  return action.type === "place" || action.type === "remove" ||
    action.type === "hint" || action.type === "reset";
}

function transition(puzzle: Puzzle, state: RuntimeState, events: readonly RuntimeEvent[]): RuntimeTransition {
  return { state, view: buildGameView(puzzle, state.history.present), events };
}

export class CrossMathGameRuntime implements CrossMathGameRuntimeContract {
  ...
  dispatch(puzzle: Puzzle, state: RuntimeState, action: RuntimeAction): RuntimeTransition {
    ...
    if (action.type === "pause" || action.type === "resume") {
      ...
      if (state.clock.paused === paused) return transition(puzzle, state, []);
      return transition(puzzle, {
        ...state,
    ...
    if (action.type === "select-tile") {
      ...
      if (state.selectedTileId === action.tileId) return transition(puzzle, state, []);
    ...
    if (action.type === "place-selected") {
      if (state.selectedTileId === null) throw new Error("No tile is selected.");
    ...
    if (!isHistoryAction(action)) return transition(puzzle, state, []);
    const beforeView = buildGameView(puzzle, state.history.present);
    const history = reduceGameHistory(puzzle, state.history, action);
    if (history === state.history) return transition(puzzle, state, []);

    const afterView = buildGameView(puzzle, history.present);
  ...
  restore(puzzle: Puzzle, serialized: string): RuntimeRestoreResult {
    ...
    if (parsed === null || typeof parsed !== "object") throw new Error("Saved runtime state is invalid.");
    const input = parsed as Partial<PersistedRuntimeState>;
    ...
    const session = restoreGameSession(puzzle, input.session);
    ...
    const state: RuntimeState = {
      schemaVersion: 1,
      puzzleId: puzzle.id,
      history: createGameHistory(puzzle, session),
      clock: input.clock,
...
export const gameRuntime = new CrossMathGameRuntime();
