import { validateLessonGuidance } from "./GuidedLessonRuntime";
...
export function countTemplateIntersections(template: LearningTopologyTemplate): number {
  const numberCoordinates = new Map<string, number>();
  for (const equation of template.equations) {
    for (const offset of [0, 2, 4] as const) {
...
export function validateLearningContent(
  ...
  for (const template of content.templates) {
    ...
    if (templateIds.has(template.id)) throw new Error(`Duplicate template: ${template.id}.`);
    ...
    for (const equation of template.equations) {
      if (equationIds.has(equation.id)) throw new Error(`Duplicate equation in ${template.id}.`);
  ...
  for (const lesson of sortedLessons) {
    ...
    if (template === undefined) throw new Error(`Missing template for lesson ${lesson.id}.`);
  ...
  if (content.campaign.schemaVersion !== 1) throw new Error("Invalid learning campaign.");
  ...
  for (const id of campaignLessonIds) {
    if (!lessonIds.has(id)) throw new Error(`Campaign references unknown lesson: ${id}.`);
  }
  return content;
}

export function buildLessonStates(
  content: LearningContent,
  library: PuzzleLibrary,
  ...
  return ordered.map((lesson) => {
    ...
    const totalStars = stars.reduce<number>((sum, value) => sum + value, 0);
    const earnedStars = Math.min(3, totalStars) as 0 | 1 | 2 | 3;
...
export function selectLessonPuzzle(
  lesson: LessonProfile,
  library: PuzzleLibrary,
  completedPuzzleIds: ReadonlySet<string> = new Set(),
) {
  ...
  if (puzzle === undefined) throw new Error(`Lesson puzzle ${id} is unavailable.`);
  return puzzle;
}
