import type { ConceptId } from "../types/LearningContent";
import type { PlayerAttemptSummary, PlayerProfileStatistics } from "../types/PremiumGameplay";

function dayKey(value: string): number {
  const parsed = Date.parse(value);
  if (Number.isNaN(parsed)) throw new Error(`Invalid completion timestamp: ${value}.`);
  return Math.floor(parsed / 86_400_000);
}

export function summarizePlayerAttempts(
  attempts: readonly PlayerAttemptSummary[],
  masteryThreshold = 3,
  ...
  const conceptCounts: Partial<Record<ConceptId, number>> = {};
  for (const attempt of completed) {
    if (attempt.concept !== undefined) conceptCounts[attempt.concept] = (conceptCounts[attempt.concept] ?? 0) + 1;
  }
  const masteredConcepts = (Object.keys(conceptCounts) as ConceptId[])
