import type {
  ...
  LearningEvent,
  LearningObjective,
  LearningRuntimeContract,
  LessonDefinition,
  LessonMetrics,
  LessonState,
  LessonTransition,
  ObjectiveProgress,
} from "../../types/LearningRuntime";
import type { RuntimeTransition } from "../../types/GameRuntime";
...
function objectiveTarget(objective: LearningObjective): number {
...
function validateObjective(objective: LearningObjective): void {
...
export function validateLessonDefinition(definition: LessonDefinition): void {
  if (definition.schemaVersion !== 1) throw new Error("Unsupported lesson schema.");
  assertId(definition.id, "Lesson ID");
  assertId(definition.title, "Lesson title");
  assertId(definition.puzzleId, "Lesson puzzle ID");
  if (definition.objectives.length === 0) throw new Error("A lesson requires at least one objective.");
...
export function validateCampaignDefinition(definition: CampaignDefinition): void {
  if (definition.schemaVersion !== 1) throw new Error("Unsupported campaign schema.");
  assertId(definition.id, "Campaign ID");
  assertId(definition.title, "Campaign title");
  if (definition.chapters.length === 0) throw new Error("A campaign requires at least one chapter.");
  ...
  for (const chapter of definition.chapters) {
    ...
    if (chapter.lessonIds.length === 0) throw new Error(`Chapter ${chapter.id} has no lessons.`);
    for (const lessonId of chapter.lessonIds) {
      if (!lessons.has(lessonId)) throw new Error(`Unknown campaign lesson: ${lessonId}.`);
      if (referenced.has(lessonId)) throw new Error(`Lesson appears more than once: ${lessonId}.`);
      referenced.add(lessonId);
    }
  }
  if (referenced.size !== lessons.size) throw new Error("Every campaign lesson must appear in exactly one chapter.");
}

function initialMetrics(): LessonMetrics {
  return {
    placedTiles: 0,
...
function initialObjectiveProgress(definition: LessonDefinition): readonly ObjectiveProgress[] {
...
function currentFor(objective: LearningObjective, metrics: LessonMetrics, puzzleCompleted: boolean): number {
  switch (objective.type) {
    case "place-tiles":
...
function computeStars(definition: LessonDefinition, progress: readonly ObjectiveProgress[]): 0 | 1 | 2 | 3 {
  const completed = progress.filter((item) => item.completed).length;
  if (completed === 0) return 0;
  if (completed === progress.length) return 3;
  if (completed * 2 >= progress.length) return 2;
  return 1;
}

function validateLessonState(definition: LessonDefinition, state: LessonState): void {
  ...
  if (![0, 1, 2, 3].includes(state.stars)) throw new Error("Saved lesson stars are invalid.");
...
function lessonOrder(definition: CampaignDefinition): readonly string[] {
  return definition.chapters.flatMap((chapter) => chapter.lessonIds);
}

function validateCampaignState(definition: CampaignDefinition, state: CampaignState): void {
  ...
  if (state.lessons.length !== order.length) throw new Error("Saved campaign lesson count is invalid.");
...
export class CrossMathLearningRuntime implements LearningRuntimeContract {
  ...
  observe(
    ...
    validateLessonState(definition, state);
    if (game.state.puzzleId !== definition.puzzleId) throw new Error("Game puzzle does not match lesson.");
    if (state.status === "completed") return { state, events: [] };
    ...
    const completedEquationIds = [...new Set([
      ...
      ...game.events
        .filter((event): event is Extract<typeof event, { type: "equation-completed" }> =>
          event.type === "equation-completed")
    ...
    for (let star = state.stars + 1; star <= stars; star += 1) {
      events.push({ type: "star-earned", lessonId: definition.id, star: star as 1 | 2 | 3 });
    }
    const completed = puzzleCompleted && objectives.every((objective) => objective.completed);
    if (completed) {
      events.push({ type: "lesson-completed", lessonId: definition.id, stars: stars as 1 | 2 | 3 });
    }
  ...
  restoreLesson(definition: LessonDefinition, serialized: string): LessonState {
    ...
    if (parsed === null || typeof parsed !== "object") throw new Error("Saved lesson state is invalid.");
    const state = parsed as LessonState;
    validateLessonState(definition, state);
    return state;
  }
  ...
  createCampaign(definition: CampaignDefinition): CampaignTransition {
    validateCampaignDefinition(definition);
    const order = lessonOrder(definition);
    const lessons: readonly CampaignLessonProgress[] = order.map((lessonId, index) => ({
      lessonId,
      unlocked: index === 0,
      completed: false,
      mastered: false,
  ...
  recordLesson(
    ...
    if (index < 0) throw new Error(`Lesson ${lesson.lessonId} is not in campaign.`);
    const lessonDefinition = definition.lessons.find((item) => item.id === lesson.lessonId);
    if (lessonDefinition === undefined) throw new Error("Campaign lesson definition is missing.");
    validateLessonState(lessonDefinition, lesson);
    if (!state.lessons[index]?.unlocked) throw new Error("Cannot record progress for a locked lesson.");

    const masteryStars = lessonDefinition.minimumStarsToMaster ?? 3;
    const lessons = state.lessons.map((item, itemIndex): CampaignLessonProgress => {
      ...
      return {
        ...
        bestStars: Math.max(item.bestStars, lesson.stars) as 0 | 1 | 2 | 3,
  ...
  restoreCampaign(definition: CampaignDefinition, serialized: string): CampaignState {
    ...
    if (parsed === null || typeof parsed !== "object") throw new Error("Saved campaign state is invalid.");
    const state = parsed as CampaignState;
    validateCampaignState(definition, state);
    return state;
  }
}

export const learningRuntime = new CrossMathLearningRuntime();
