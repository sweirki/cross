...
export type ClusterTransform = ClusterTemplate["allowedTransforms"][number];
type Direction = ClusterPort["direction"];

function transformPoint(row: number, col: number, transform: ClusterTransform) {
  switch (transform) {
...
function transformDirection(direction: Direction, transform: ClusterTransform): Direction {
  ...
  if (point.row === -1 && point.col === 0) return "north";
  if (point.row === 0 && point.col === 1) return "east";
  if (point.row === 1 && point.col === 0) return "south";
  return "west";
}

export function transformClusterTemplate(
  ...
  const equations: ClusterEquationPath[] = template.equations.map((equation) => {
    ...
    return {
      ...equation,
      orientation,
      cellIds: ordered as [string, string, string, string, string],
    };
  });

  const expectedKinds = ["number", "operator", "number", "equals", "number"] as const;
  const kindById = new Map<string, ClusterCell["kind"]>();
  ...
  const cells: ClusterCell[] = template.cells.map((cell) => ({
    id: cell.id,
...
function geometrySignature(template: ClusterTemplate): string {
  const positionById = new Map(
    template.cells.map((cell) => [cell.id, `${cell.position.row},${cell.position.col}`] as const),
  );
...
export function canonicalClusterSignature(template: ClusterTemplate): string {
