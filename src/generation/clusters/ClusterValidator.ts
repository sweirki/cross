...
const EXPECTED_KINDS: readonly ClusterCellKind[] = [
  "number",
  "operator",
  "number",
  "equals",
...
export function validateClusterTemplate(template: ClusterTemplate): ClusterValidationResult {
  const errors: string[] = [];
  if (template.schema !== GENERATION_SCHEMA_IDS.clusterTemplate) {
    errors.push("wrong cluster-template schema");
  }
  if (template.id.trim() === "") errors.push("template id is empty");
  if (template.canonicalId.trim() === "") errors.push("canonical id is empty");
  if (template.cells.length === 0) errors.push("template has no cells");
  if (template.equations.length < 2) errors.push("template must contain at least two equations");

  const cellsById = new Map<string, (typeof template.cells)[number]>();
  const cellsByPosition = new Map<string, (typeof template.cells)[number]>();
  for (const cell of template.cells) {
    if (cellsById.has(cell.id)) errors.push(`duplicate cell id ${cell.id}`);
    cellsById.set(cell.id, cell);
    const key = positionKey(cell.position.row, cell.position.col);
    if (cellsByPosition.has(key)) errors.push(`duplicate cell position ${key}`);
  ...
  for (const equation of template.equations) {
    if (equationIds.has(equation.id)) errors.push(`duplicate equation id ${equation.id}`);
    ...
    const resolved = cells as NonNullable<(typeof cells)[number]>[];
    ...
    for (let index = 1; index < resolved.length; index += 1) {
      ...
      if (!expected) errors.push(`equation ${equation.id} is not contiguous`);
    }
  }
  ...
  const equationAdjacency = new Map<string, Set<string>>();
  for (const equation of template.equations) equationAdjacency.set(equation.id, new Set());
  ...
  if (template.equations.length > 0) {
    ...
    while (pending.length > 0) {
      ...
      visited.add(current);
      for (const neighbor of equationAdjacency.get(current) ?? []) pending.push(neighbor);
    }
    if (visited.size !== template.equations.length) errors.push("equation graph is disconnected");
  }

  const portIds = new Set<string>();
  for (const port of template.ports) {
    if (portIds.has(port.id)) errors.push(`duplicate port id ${port.id}`);
    portIds.add(port.id);
    const cell = cellsById.get(port.cellId);
    if (!cell) errors.push(`port ${port.id} references an unknown cell`);
    else if (cell.kind !== "number") errors.push(`port ${port.id} must reference a number cell`);
  }
...
export function assertValidClusterTemplate(template: ClusterTemplate): void {
