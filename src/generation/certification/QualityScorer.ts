...
import type { CertificationInput, QualityScorecard } from "./CertificationTypes";
...
function metric(record: Readonly<Record<string, number>>, name: string, fallback = 0): number {
  const value = record[name];
  return value === undefined || !Number.isFinite(value) ? fallback : value;
}

export function scoreCandidate(input: CertificationInput): QualityScorecard {
  ...
  const tuning = productionTuningProfile(candidate.request.difficulty);
  ...
  const difficultyAccuracy = clamp(
    depthFitness * 0.5 +
    startFitness * 0.3 +
