import type {
  DifficultyTelemetryEvent,
  DifficultyTelemetrySession,
  DifficultyTelemetrySummary,
  TelemetryPrivacyValidation,
} from "./TelemetryTypes";
...
export function validateTelemetryPrivacy(value: unknown): TelemetryPrivacyValidation {
  ...
  const visit = (node: unknown, path: string): void => {
    ...
    for (const [key, child] of Object.entries(node as Record<string, unknown>)) {
      ...
      if (FORBIDDEN_KEYS.has(key)) forbiddenPaths.push(childPath);
...
function friction(events: readonly DifficultyTelemetryEvent[]): Readonly<Record<string, number>> {
  const values: Record<string, number> = {};
  for (const event of events) {
    const step = "deductionStepIndex" in event ? event.deductionStepIndex : undefined;
    if (step === undefined) continue;
    let weight = 0;
    if (event.type === "cell-placed" && !event.correct) weight = 2;
    if (event.type === "hint-requested") weight = event.level;
    if (event.type === "stall") weight = Math.max(1, Math.ceil(event.durationMs / 15000));
    if (weight > 0) values[String(step)] = (values[String(step)] ?? 0) + weight;
  }
  return Object.freeze(Object.fromEntries(Object.entries(values).sort(([a], [b]) => Number(a) - Number(b))));
}

export function summarizeDifficultyTelemetry(session: DifficultyTelemetrySession): DifficultyTelemetrySummary {
  ...
  const terminal = [...events].reverse().find((event) =>
    event.type === "session-completed" || event.type === "session-abandoned",
  );
  const placements = events.filter((event) => event.type === "cell-placed");
  const hints = events.filter((event) => event.type === "hint-requested");
  const stalls = events.filter((event): event is Extract<DifficultyTelemetryEvent, { type: "stall" }> => event.type === "stall");
...
export function appendTelemetryEvent(
  session: DifficultyTelemetrySession,
  event: DifficultyTelemetryEvent,
): DifficultyTelemetrySession {
  if (!Number.isFinite(event.atMs) || event.atMs < 0) throw new Error("INVALID_EVENT_TIMESTAMP");
  return Object.freeze({
    ...session,
