import type { DeductionStep } from "../contracts/GenerationContracts";
import type { HintEscalationLevel, HintRequest, HintResult, HintValidationResult } from "./HintTypes";

function clampLevel(value: number | undefined): HintEscalationLevel {
  if (value === undefined || value <= 1) return 1;
  if (value >= 4) return 4;
  return value as HintEscalationLevel;
}
...
function findNextStep(request: HintRequest): DeductionStep | undefined {
  const solved = new Set(request.solvedCellIds);
  return request.trace.steps.find((step) =>
    !solved.has(step.cellId) &&
    step.prerequisiteCellIds.every((cellId) => solved.has(cellId)),
  );
}

export function createHint(request: HintRequest): HintResult {
...
export function validateHintAgainstTrace(request: HintRequest, hint: HintResult): HintValidationResult {
  const errors: string[] = [];
  if (hint.puzzleId !== request.puzzleId) errors.push("PUZZLE_ID_MISMATCH");
  if (hint.kind === "reveal-value") {
    const step = request.trace.steps.find((candidate) => candidate.index === hint.stepIndex);
    if (!step) errors.push("UNKNOWN_DEDUCTION_STEP");
    else {
      if (step.cellId !== hint.targetCellId) errors.push("TARGET_CELL_MISMATCH");
      if (step.value !== hint.revealedValue) errors.push("REVEALED_VALUE_MISMATCH");
    }
