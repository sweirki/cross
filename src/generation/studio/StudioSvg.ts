import type { StudioInspection } from "./StudioTypes";

function escape(value: string): string {
  return value.replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll('"', "&quot;");
}

export function renderCandidateSvg(inspection: StudioInspection): string {
  const candidate = inspection.candidate;
  if (!candidate) throw new Error("Cannot render a candidate that was not generated.");
