...
const EMPTY_STATS: ProgressionStatistics = {
  puzzlesCompleted: 0,
  perfectSolves: 0,
  hintFreeSolves: 0,
...
function assertDateTime(value: string): void {
  ...
  if (!Number.isFinite(date.getTime())) throw new Error("Completion date is invalid.");
}
...
function rewardId(revision: number, index: number): string {
  return `reward-${revision}-${index}`;
}

export class CrossMathProgressionRuntime {
  public create(playerId: string): ProgressionState {
  ...
  public levelForXp(xp: number): number {
    ...
    let level = 1;
    while (this.xpForLevel(level + 1) <= xp) level += 1;
    return level;
  }

  public levelProgress(state: ProgressionState): {
  ...
  public baseXp(input: ProgressionCompletionInput): number {
    this.validateInput(input);
    let xp = 50 + input.stars * 25;
    if (input.hintsUsed === 0) xp += 15;
    if (input.mistakes === 0) xp += 15;
    if (input.elapsedMs <= 60_000) xp += 20;
    if (input.mode === "daily") xp += 30;
    if (input.lessonCompleted === true) xp += 40;
    if (input.campaignCompleted === true) xp += 150;
    if (input.masteryImproved === true) xp += 25;
    return xp;
  }

  public completionId(input: ProgressionCompletionInput): string {
    return `${input.puzzleId}|${input.completedAt}|${input.mode}`;
  }

  public recordCompletion(state: ProgressionState, input: ProgressionCompletionInput): ProgressionResult {
    this.validateInput(input);
    ...
    if (state.lastActiveDate === null) currentStreak = 1;
    else if (activityDate === state.lastActiveDate) currentStreak = state.currentStreak;
    else {
    ...
    const stats: ProgressionStatistics = {
  ...
  public dismissReward(state: ProgressionState, rewardIdValue?: string): ProgressionState {
    if (state.rewardQueue.length === 0) return state;
    const id = rewardIdValue ?? state.rewardQueue[0]!.id;
    if (!state.rewardQueue.some(item => item.id === id)) return state;
    return {
      ...state,
  ...
  public clearRewards(state: ProgressionState): ProgressionState {
    if (state.rewardQueue.length === 0) return state;
    return { ...state, rewardQueue: [], revision: state.revision + 1 };
  }

  public averageSolveTimeMs(state: ProgressionState): number | null {
    return state.stats.puzzlesCompleted === 0
      ? null
      : Math.round(state.stats.totalPlayTimeMs / state.stats.puzzlesCompleted);
  }

  public achievement(id: AchievementId): AchievementDefinition {
    const found = ACHIEVEMENTS.find(item => item.id === id);
    if (found === undefined) throw new Error(`Unknown achievement: ${id}`);
    return found;
  }

  public serialize(state: ProgressionState): string {
    return JSON.stringify(canonical(state));
  }

  public restore(playerId: string, serialized: string): ProgressionState {
    let parsed: unknown;
    try { parsed = JSON.parse(serialized); } catch { throw new Error("Progression data is not valid JSON."); }
    if (parsed === null || typeof parsed !== "object") throw new Error("Progression data is invalid.");
    const state = parsed as ProgressionState;
    if (state.schemaVersion !== 1 || state.playerId !== playerId) throw new Error("Progression data is incompatible.");
    assertNonNegativeInteger(state.totalXp, "Total XP");
    assertNonNegativeInteger(state.revision, "Revision");
    if (state.level !== this.levelForXp(state.totalXp)) throw new Error("Progression level is inconsistent.");
    if (!Array.isArray(state.achievements) || !Array.isArray(state.rewardQueue)) throw new Error("Progression arrays are invalid.");
    return state;
  }

  private validateInput(input: ProgressionCompletionInput): void {
    if (input.puzzleId.trim().length === 0) throw new Error("Puzzle ID must not be empty.");
    assertDateTime(input.completedAt);
    if (![1, 2, 3].includes(input.stars)) throw new Error("Stars must be between one and three.");
  ...
  private achievementCandidates(
    stats: ProgressionStatistics,
    streak: number,
    input: ProgressionCompletionInput,
  ): AchievementId[] {
    const ids: AchievementId[] = [];
    if (stats.puzzlesCompleted >= 1) ids.push("first-solve");
    if (stats.puzzlesCompleted >= 5) ids.push("five-solves");
    if (stats.puzzlesCompleted >= 25) ids.push("twenty-five-solves");
    if (input.stars === 3) ids.push("perfect-solve");
    if (input.hintsUsed === 0) ids.push("hint-free");
    if (input.elapsedMs <= 60_000) ids.push("fast-solve");
    if (streak >= 3) ids.push("daily-streak-3");
    if (streak >= 7) ids.push("daily-streak-7");
    if (stats.totalStars >= 25) ids.push("star-collector-25");
    if (stats.totalStars >= 100) ids.push("star-collector-100");
    return ids;
  }
}

export const progressionRuntime = new CrossMathProgressionRuntime();
