import type { Breakpoint, LayoutMetrics } from "../types";
...
export function resolveLayoutMetrics(width: number, fontScale = 1, responsiveType = true): LayoutMetrics {
  if (!Number.isFinite(fontScale) || fontScale <= 0) throw new Error("Font scale must be positive.");
